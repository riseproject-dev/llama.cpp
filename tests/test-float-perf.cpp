...
template <typename T> static void generate_vector(T * data, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            data[i] = ggml_compute_fp32_to_fp16(f_val);
        } else if constexpr (std::is_same_v<T, ggml_bf16_t>) {
            data[i] = ggml_compute_fp32_to_bf16(f_val);
        } else {
...
static void benchmark_kernel(size_t  size,
    ...
    for (int i = 0; i < iterations; i++) {
        const int64_t start_time = ggml_time_ns();
        func();
        const int64_t end_time = ggml_time_ns();
...
int main() {
    ...
    ggml_time_init();
    {
        ...
        for (size_t size : test_sizes) {
            int64_t       ops   = 2 * size;
            ggml_fp16_t * x_f16 = (ggml_fp16_t *) ggml_aligned_malloc(size * sizeof(ggml_fp16_t));
            ggml_fp16_t * y_f16 = (ggml_fp16_t *) ggml_aligned_malloc(size * sizeof(ggml_fp16_t));
            generate_vector<ggml_fp16_t>(x_f16, size);
            generate_vector<ggml_fp16_t>(y_f16, size);
            auto opt_fn = [&]() {
                ggml_get_type_traits_cpu(GGML_TYPE_F16)->vec_dot(size, &result, 0, x_f16, 0, y_f16, 0, 1);
                return result;
            };
            benchmark_kernel(size, ITERATIONS, ops, opt_fn);
            ggml_aligned_free(x_f16, size * sizeof(ggml_fp16_t));
            ggml_aligned_free(y_f16, size * sizeof(ggml_fp16_t));
        }
    }

    {
        ...
        for (size_t size : test_sizes) {
            ...
            ggml_fp16_t * xv_f16_unroll =
                (ggml_fp16_t *) ggml_aligned_malloc(GGML_VEC_DOT_UNROLL * size * sizeof(ggml_fp16_t));
            ggml_fp16_t * y_f16 = (ggml_fp16_t *) ggml_aligned_malloc(size * sizeof(ggml_fp16_t));
            ...
            benchmark_kernel(size, ITERATIONS, ops, opt_fn);
            ggml_aligned_free(xv_f16_unroll, GGML_VEC_DOT_UNROLL * size * sizeof(ggml_fp16_t));
            ggml_aligned_free(y_f16, size * sizeof(ggml_fp16_t));
        }
    }
...

    {
        ...
        for (size_t size : test_sizes) {
            int64_t       ops    = size;
            ggml_bf16_t * x_bf16 = (ggml_bf16_t *) ggml_aligned_malloc(size * sizeof(ggml_bf16_t));
            float *       y_f32  = (float *) ggml_aligned_malloc(size * sizeof(float));
            ...
            benchmark_kernel(size, ITERATIONS, ops, opt_fn);
            ggml_aligned_free(x_bf16, size * sizeof(ggml_bf16_t));
            ggml_aligned_free(y_f32, size * sizeof(float));
        }
    }
...

    {
        ...
        for (size_t size : test_sizes) {
            int64_t ops   = 33 * size;
            float * y_f32 = (float *) ggml_aligned_malloc(size * sizeof(float));
            float * x_f32 = (float *) ggml_aligned_malloc(size * sizeof(float));
            ...
            benchmark_kernel(size, ITERATIONS, ops, opt_fn);
            ggml_aligned_free(x_f32, size * sizeof(float));
            ggml_aligned_free(y_f32, size * sizeof(float));
        }
    }
    return 0;
