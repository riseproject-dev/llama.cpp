...
#define DOT_F16_ERROR_THRESHOLD   0.4
#define DOT_BF16_ERROR_THRESHOLD  0.04f
#define MAD_F16_ERROR_THRESHOLD   0.04f
#define SCALE_F16_ERROR_THRESHOLD 0.04f
#define DOT_F16_UNROLL_THRESHOLD  0.4
...
static inline void ggml_vec_mad_f16_reference(const int                         n,
                                              ...
                                              const float                       v) {
    for (int i = 0; i < n; ++i) {
        y[i] = ggml_compute_fp32_to_fp16(ggml_compute_fp32_to_fp16(y[i]) + ggml_compute_fp32_to_fp16(x[i]) * v);
    }
}
...
static inline void ggml_vec_scale_f16_reference(const int n, ggml_fp16_t * y, const float v) {
    ...
    // On ARM, scaling happens in FP16.
    const _Float16 vf = (_Float16)v;
    for (int i = 0; i < n; ++i) {
        _Float16 yf; memcpy(&yf, &y[i], sizeof(yf));
        yf = yf * vf;
        memcpy(&y[i], &yf, sizeof(yf));
    }
#else
    for (int i = 0; i < n; ++i) {
        y[i] = ggml_compute_fp32_to_fp16(ggml_compute_fp32_to_fp16(y[i]) * v);
    }
#endif
}
...
static void ggml_vec_dot_f16_reference(int                         n,
    ...
    GGML_UNUSED(bs);
#if defined(__aarch64__)
    // On ARM, the dot product accumulator and reduction happens in FP16 (svaddv_f16).
    // The reference must accumulate in _Float16 to match the precision loss/behavior.
    _Float16 sumf = 0.0f;

    for (int i = 0; i < n; ++i) {
        _Float16 xf; memcpy(&xf, &x[i], sizeof(xf));
        _Float16 yf; memcpy(&yf, &y[i], sizeof(yf));
        sumf += xf * yf;
    }
    ...
    *s = (float)sumf;
#else
    // Standard implementation: accumulates in FP32
    ggml_float sumf = 0.0;

    for (int i = 0; i < n; ++i) {
        sumf += (ggml_float) (ggml_compute_fp32_to_fp16(x[i]) * ggml_compute_fp32_to_fp16(y[i]));
    }
...
inline static void ggml_vec_dot_f16_unroll_reference(const int                   n,
...
#if defined(__aarch64__)
    // On ARM, the optimized kernel uses FP16 accumulators (svfloat16_t)
    // and FP16 arithmetic.
    _Float16 sumf[GGML_VEC_DOT_UNROLL] = { 0.0f };

    for (int i = 0; i < n; ++i) {
        _Float16 y_val; memcpy(&y_val, &y[i], sizeof(y_val));

        for (int j = 0; j < GGML_VEC_DOT_UNROLL; ++j) {
            _Float16 x_val; memcpy(&x_val, &x[j][i], sizeof(x_val));
            sumf[j] += x_val * y_val;
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < GGML_VEC_DOT_UNROLL; ++j) {
            sumf[j] += (ggml_float) (ggml_compute_fp32_to_fp16(x[j][i]) * ggml_compute_fp32_to_fp16(y[i]));
        }
    }
...
static void validate_dot_fp16(int n, std::vector<ggml_fp16_t> & x, std::vector<ggml_fp16_t> & y, TestStats & stats) {
    ...
    ggml_get_type_traits_cpu(GGML_TYPE_F16)->vec_dot(n, &s_opt, 0, x.data(), 0, y.data(), 0, 1);
...
static void validate_dot_bf16(int n, std::vector<ggml_bf16_t> & x, std::vector<ggml_bf16_t> & y, TestStats & stats) {
    ...
    ggml_get_type_traits_cpu(GGML_TYPE_BF16)->vec_dot(n, &s_opt, 0, x.data(), 0, y.data(), 0, 1);
...
static void test_vec_scale_f16_all_cases(TestStats & global_stats) {
    ...
    const float       scalars[]  = { 0.0f, 1.0f, -1.0f, 3.14f};
